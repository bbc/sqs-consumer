...
- By default, messages that are sent to the `handleMessage` and `handleMessageBatch` will be considered as processed if the provided functions return without an error. To acknowledge individual messages, please return the message that you want to acknowledge if you are using `handleMessage` or the messages for `handleMessageBatch`
