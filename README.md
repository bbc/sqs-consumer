...
* `handleMessagesBatch` - _Function_ - A function to be called whenever a batch of messages is received. Similar to `handleMessage` but will receive the list of messages, not each message individually. **If both are set, `handleMessagesBatch` overrides `handleMessage`**.
